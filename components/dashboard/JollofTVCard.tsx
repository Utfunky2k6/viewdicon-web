...
export default function JollofTVCard() {
  return (
    <div style={{ margin:'8px 12px', borderRadius:14, overflow:'hidden', cursor:'pointer', position:'relative' }}>
        ...
        <div style={{ position:'relative', zIndex:1, display:'flex', alignItems:'flex-end', justifyContent:'space-between', width:'100%' }}>
          ...
          <div style={{ display:'flex', gap:5 }}>
            {['▶ Watch','🏺 Spray','⭐ Kíla'].map(lbl => (
              <span key={lbl} style={{ background:'rgba(255,255,255,.15)', border:'1px solid rgba(255,255,255,.25)', borderRadius:99, padding:'4px 9px', fontSize:10, fontWeight:700, color:'#fff', cursor:'pointer' }}>{lbl}</span>
            ))}
          </div>
        </div>
      </div>
