...
export default function NationFires({ mode }: { mode: ThemeMode }) {
  ...
  return (
    <>
      <SectionLabel label="🔥 Nation's Fires" more="See all" mode={mode} />
      <div style={{ display:'flex', gap:8, padding:'0 12px', overflowX:'auto', scrollbarWidth: 'none' }}>
        ...
        {fires.map((f, i) => {
          ...
          return (
            <div key={f.id || i} style={{ minWidth:108, borderRadius:12, padding:'11px 10px', flexShrink:0, cursor:'pointer', background:bg, boxShadow: '0 4px 12px rgba(0,0,0,0.2)' }}>
