...
const REELS = [
  { id:'r1', type:'market',    emoji:'👗', color:'#e07b00', colorBg:'rgba(224,123,0,.15)',   streamer:'Mama Adaeze', village:'🧺 Ìlú Oníṣòwò', viewers:'1,421', label:'🧺 Commerce · Station B', desc:'Live fashion show — Ankara collection 2026. 10 yards at ₡4,500 NOW.', product:'Ankara Fabric · 6 yards', price:'₡ 4,500', kila:234, spray:89, drum:45 },
  { id:'r2', type:'farm',      emoji:'🌾', color:'#1a7c3e', colorBg:'rgba(26,124,62,.15)',    streamer:'Kofi Mensah · Mwea Farm', village:'🌾 Ìlú Àgbẹ̀', viewers:'334', label:'🌾 Agriculture · Station B', desc:'Live harvest auction — 850 bags of maize. Market price showing NOW.', kila:67, spray:22, drum:31 },
  { id:'r3', type:'health',    emoji:'⚕',  color:'#0369a1', colorBg:'rgba(3,105,161,.15)',    streamer:'Dr. Ngozi Eze', village:'⚕ Ìlú Ìwòsàn', viewers:'892', label:'⚕ Health · Station B', desc:'Understanding malaria in children under 5. LIVE translation in 5 languages.', sv:'Spirit Voice: Auto-translating to YO · IG · HA · SW', kila:445, spray:0, drum:167 },
  { id:'r4', type:'oracle',    emoji:'⚡', color:'#d4a017', colorBg:'rgba(212,160,23,.15)',   streamer:'Oracle Council · All Villages', village:'🏛 Ìlú Ìgbìmọ', viewers:'4,821', label:'🏛 Nation · Station B', desc:'LIVE: Should agriculture villages set cocoa price floors across West Africa?', topic:'"West Africa Cocoa Price Floor"', agree:68, kila:890, spray:340, drum:445 },
  { id:'r5', type:'craft',     emoji:'🧵', color:'#7c3aed', colorBg:'rgba(124,58,237,.15)',   streamer:'Kofi the Kente Master', village:'🎨 Ìlú Ọnà', viewers:'2,147', label:'🎨 Arts · Station B', desc:'Kente cloth weaving — 78% complete. Commission this piece or order yours.', progress:78, kila:1200, spray:567, drum:234 },
  { id:'r6', type:'knowledge', emoji:'📚', color:'#4f46e5', colorBg:'rgba(79,70,229,.15)',    streamer:'Prof. Amara Osei', village:'🎓 Ìlú Ẹ̀kọ', viewers:'2,100', label:'🎓 Education · Station B', desc:'African Mathematics: Yoruba counting predates Roman numerals by 1,000 years.', sv:'"Ẹgbàáàdọrin" = 80 in Yoruba (4×20). The vigesimal system of ancient Africa.', kila:678, spray:89, drum:312 },
]
...
function GuideScreen({ onStation }: { onStation:(s:StationView)=>void }) {
  const STATIONS = [
    ...
    { id:'stationC' as StationView, emoji:'📺', num:'Station C', name:'Jollof Broadcast',now:'Oracle Session LIVE',      viewers:'1.2K', color:'#d4a017' },
  ]
  ...
  return (
    <div style={{ flex:1,overflowY:'auto' }}>
      ...
      <div style={{ display:'flex',gap:6,padding:'0 14px',marginBottom:12 }}>
        ...
        <button onClick={()=>onStation('stationC')} style={{ flex:1,padding:'11px',borderRadius:11,background:'rgba(212,160,23,.15)',border:'1px solid rgba(212,160,23,.25)',color:'#fbbf24',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>📋 Full Programme Guide</button>
      </div>
      ...
      {SCHEDULE.map((p,i)=>{
        ...
        return (
          <div key={i} onClick={()=>{}} style={{ display:'flex',gap:8,padding:'4px 14px',borderBottom:'1px solid rgba(255,255,255,.04)',cursor:'pointer',transition:'background .15s' }}>
            ...
            <div style={{ flexShrink:0,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4,paddingTop:6 }}>
              {isNow ? <span style={{ fontSize:8,fontWeight:700,background:'#ef4444',color:'#fff',borderRadius:99,padding:'2px 7px' }}>ON AIR</span> : <span style={{ fontSize:10,color:'rgba(255,255,255,.3)',cursor:'pointer' }}>🔔</span>}
    ...
    <div style={{ display:'flex',flexDirection:'column',flex:1,overflow:'hidden',background:'#070900' }}>
      ...
      <div style={{ position:'relative',background:'#000',flexShrink:0 }}>
        ...
        <div style={{ display:'flex',gap:4,padding:'6px 8px',background:'rgba(0,0,0,.85)' }}>
          ...
          <div onClick={()=>showToast('🤚 Join request sent to Mama Ngozi — queue: 4')} style={{ flex:1,height:60,borderRadius:8,border:'1.5px dashed rgba(255,255,255,.1)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:2,cursor:'pointer' }}>
            <span style={{ fontSize:18 }}>🤚</span><span style={{ fontSize:8,fontWeight:700,color:'rgba(255,255,255,.4)',textAlign:'center' }}>Request to Join</span>
          </div>
        </div>
      </div>
      {/* live ticker */}
        ...
        <div style={{ margin:'8px 0',background:'linear-gradient(135deg,#1a1800,#1a1200)',border:'1px solid rgba(212,160,23,.2)',borderRadius:12,overflow:'hidden' }}>
          ...
          <div style={{ padding:'10px 12px',display:'flex',gap:10,alignItems:'center' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <button onClick={()=>showToast('🏦 Access Bank loan application opened...')} style={{ marginTop:8,padding:'7px 14px',background:'#d4a017',border:'none',borderRadius:8,fontSize:11,fontWeight:700,color:'#111',cursor:'pointer',fontFamily:'Sora,sans-serif' }}>Apply via Viewdicon →</button>
            </div>
          </div>
        </div>
        <div onClick={()=>showToast('📢 Book ad slot: ₡5,000/30s · village@jollof.tv')} style={{ textAlign:'center',fontSize:10,color:'rgba(212,160,23,.4)',cursor:'pointer',padding:'6px 0' }}>📢 Want to advertise here? Book a TV slot →</div>
      </div>
      {/* react strip */}
      <div style={{ display:'flex',gap:6,padding:'8px 12px',background:'rgba(0,0,0,.85)',borderTop:'1px solid rgba(255,255,255,.05)',flexShrink:0 }}>
        {[['💸 Spray ₡','rgba(212,160,23,.2)','#fbbf24','rgba(212,160,23,.3)',doSpray],['✓ Vote','rgba(26,124,62,.15)','#4ade80','rgba(26,124,62,.25)',()=>showToast('✓ Voted — 69% now agree')],['🤚 Join Stage','#e07b00','#fff','#e07b00',()=>showToast('🤚 Join request sent — queue: 4')],['🥁','rgba(255,255,255,.07)','rgba(255,255,255,.5)','rgba(255,255,255,.1)',()=>showToast('🥁 Drummed to Village!')]].map(([l,bg,c,b,fn],i)=>(
          ...
          return (
            <div key={reel.id} onClick={()=>setActiveReel(idx)} style={{ height:'calc(100dvh - 130px)',scrollSnapAlign:'start',position:'relative',overflow:'hidden',background:`linear-gradient(160deg,${reel.colorBg},#070900 60%)`,flexShrink:0,cursor:'pointer' }}>
              ...
              <div style={{ position:'absolute',right:12,bottom:180,display:'flex',flexDirection:'column',gap:14,zIndex:10 }}>
                {[['⭐',reel.kila.toString(),'Kíla'],['💸',reel.spray.toString(),'Spray'],['🥁',reel.drum.toString(),'Drum'],['💬','Ask','Chat'],['↗','Share','']].map(([ico,cnt,lbl],i)=>(
                  <div key={i} onClick={e=>{e.stopPropagation(); if(i===1)doSpray(reel.id); else showToast(lbl==='Kíla'?'⭐ Kíla given!':lbl==='Drum'?'🥁 Drummed!':lbl==='Chat'?'💬 Comment':lbl===''?'🔗 Shared!':'')}} style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:4,cursor:'pointer' }}>
              ...
              <div style={{ position:'absolute',bottom:0,left:0,right:0,padding:'12px 14px 14px',background:'linear-gradient(transparent,rgba(0,0,0,.92)',zIndex:5 }}>
                ...
                {reel.type==='market' && reel.product && (
                  <div style={{ background:'rgba(0,0,0,.85)',borderRadius:12,padding:'10px 12px',marginBottom:8 }}>
                    ...
                    <div style={{ display:'flex',gap:6 }}>
                      <button onClick={e=>{e.stopPropagation();showToast('🛒 Business Session opening!')}} style={{ flex:1,padding:9,borderRadius:9,background:'#e07b00',border:'none',color:'#fff',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>🛒 BUY NOW</button>
                      <button onClick={e=>{e.stopPropagation();showToast('💬 Message sent')}} style={{ flex:1,padding:9,borderRadius:9,background:'rgba(255,255,255,.12)',border:'none',color:'#fff',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>Ask Seller</button>
                    </div>
                  </div>
                )}
                {/* farm auction */}
                {reel.type==='farm' && (
                  <div style={{ background:'rgba(0,0,0,.85)',borderRadius:12,padding:'10px 12px',marginBottom:8 }}>
                    ...
                    <button onClick={e=>{e.stopPropagation();showToast('🌾 Bid placed! Leading auction. Escrow ready.')}} style={{ width:'100%',padding:10,background:'linear-gradient(135deg,#1a7c3e,#0f5028)',border:'none',borderRadius:9,fontSize:12,fontWeight:700,color:'#fff',cursor:'pointer',fontFamily:'Sora,sans-serif',marginTop:4 }}>🌾 Place Your Bid</button>
                  </div>
                )}
                {/* oracle vote */}
                {reel.type==='oracle' && reel.topic && (
                  <div style={{ background:'rgba(0,0,0,.85)',borderRadius:12,padding:'10px 12px',marginBottom:8 }}>
                    ...
                    <div style={{ display:'flex',gap:5 }}>
                      <button onClick={e=>{e.stopPropagation();setAgree(a=>Math.min(95,a+3));showToast('✓ Agree!')}} style={{ flex:1,padding:8,borderRadius:9,border:'1px solid rgba(26,124,62,.3)',background:'rgba(26,124,62,.2)',color:'#4ade80',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>✓ Agree</button>
                      <button onClick={e=>{e.stopPropagation();setAgree(a=>Math.max(5,a-3));showToast('✗ Disagree!')}} style={{ flex:1,padding:8,borderRadius:9,border:'1px solid rgba(178,34,34,.25)',background:'rgba(178,34,34,.15)',color:'#f87171',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>✗ Disagree</button>
                      <button onClick={e=>{e.stopPropagation();showToast('🤚 Hand raised — queue 7')}} style={{ flex:1,padding:8,borderRadius:9,border:'1px solid rgba(212,160,23,.25)',background:'rgba(212,160,23,.15)',color:'#fbbf24',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Sora,sans-serif' }}>🤚 Speak</button>
                    </div>
                  </div>
                )}
                {/* craft progress */}
                {reel.type==='craft' && reel.progress && (
                  <div style={{ background:'rgba(0,0,0,.85)',borderRadius:12,padding:'10px 12px',marginBottom:8 }}>
                    ...
                    <button onClick={e=>{e.stopPropagation();showToast('🧵 Commission opened — Chat with Kofi')}} style={{ width:'100%',padding:10,background:'linear-gradient(135deg,#7c3aed,#5b21b6)',border:'none',borderRadius:9,fontSize:12,fontWeight:700,color:'#fff',cursor:'pointer',fontFamily:'Sora,sans-serif' }}>🧵 Commission This Piece</button>
                  </div>
                )}
                {/* spirit voice */}
                ...
                <div style={{ display:'flex',alignItems:'center',gap:9,marginBottom:8 }}>
                  ...
                  <div onClick={e=>{e.stopPropagation();showToast(`🏘 Whisper sent to ${reel.streamer}`)}} style={{ padding:'6px 14px',borderRadius:99,border:'1.5px solid #fff',fontSize:10,fontWeight:700,color:'#fff',cursor:'pointer',flexShrink:0 }}>🤫 Whisper</div>
                </div>
...
export default function JollofPage() {
  ...
  const SB_LABEL: Record<StationView,string> = {
    guide:'📡 NOW LIVE', stationA:'🎭 Station A', stationB:'📡 Station B', stationC:'📺 Station C',
  }
    ...
    <div style={{ height:'100dvh',background:'#07090a',color:'#f0f5ee',fontFamily:'DM Sans,sans-serif',display:'flex',flexDirection:'column',overflow:'hidden' }}>
      ...
      <div style={{ background:'#0c1009',padding:'10px 14px 0',flexShrink:0 }}>
        <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:10 }}>
          ...
          <div style={{ display:'flex',gap:6 }}>
            {['🔔','⚙'].map((ico,i)=><div key={i} style={{ width:32,height:32,borderRadius:'50%',background:'rgba(255,255,255,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,cursor:'pointer' }}>{ico}</div>)}
          </div>
        </div>
        {/* station tabs on guide */}
        {view==='guide' && (
          <div style={{ display:'flex',gap:5,paddingBottom:8 }}>
            {(['stationA','stationB','stationC'] as StationView[]).map((s,i)=>(
              <button key={s} onClick={()=>setView(s)} style={{ padding:'5px 10px',borderRadius:8,background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.2)',fontSize:9,fontWeight:700,color:'#f87171',cursor:'pointer' }}>{'ABC'[i]}</button>
            ))}
          </div>
        )}
      </div>
