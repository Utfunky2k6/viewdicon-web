    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a0a', color: '#fff', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '15px 20px', background: '#111', borderBottom: '1px solid #262626', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 15 }}>
          <button style={{ background: 'transparent', border: 'none', color: '#7c3aed', fontSize: 20, cursor: 'pointer' }}>🌳</button>
          <button style={{ background: 'transparent', border: 'none', color: '#fff', fontSize: 20, cursor: 'pointer' }}>📞</button>
        </div>
      </div>
          ...
          if (msg.type === 'blood_call') {
            return (
              <div key={i} style={{ alignSelf: 'center', width: '100%', maxWidth: 400, background: 'linear-gradient(135deg, #7f1d1d, #b91c1c)', borderRadius: 16, padding: '16px 20px', boxShadow: '0 0 20px rgba(220, 38, 38, 0.4)', animation: 'pulse 2s infinite' }}>
                ...
                <button style={{ width: '100%', padding: 12, background: '#fff', color: '#b91c1c', border: 'none', borderRadius: 99, fontWeight: 800, cursor: 'pointer' }}>View Live GPS & Respond</button>
              </div>
            )
          }
            ...
            return (
              <div key={i} style={{ alignSelf: 'center', width: '100%', maxWidth: 400, background: '#1e293b', border: '1px solid #334155', borderRadius: 16, padding: '16px 20px' }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button style={{ flex: 1, padding: 10, background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>Turn Vault Key</button>
                  <button style={{ flex: 1, padding: 10, background: '#0f172a', color: '#ef4444', border: '1px solid #334155', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>Deny Access</button>
                </div>
              </div>
            )
          }
              ...
              <div key={i} style={{ alignSelf: 'center', width: '100%', maxWidth: 400, background: '#4c1d95', border: '1px solid #7c3aed', borderRadius: 16, padding: '16px 20px', boxShadow: '0 4px 20px rgba(124,58,237,0.2)' }}>
                ...
                {recoveryStatus === 'pending' && !showRecoveryForm && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <button style={{ padding: 12, background: 'rgba(220, 38, 38, 0.15)', color: '#ef4444', border: '1px solid rgba(220, 38, 38, 0.3)', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>Report Suspicious</button>
                  </div>
                )}
      ...
      <div style={{ padding: 20, background: '#111', borderTop: '1px solid #262626', display: 'flex', gap: 10 }}>
        <button style={{ width: 44, height: 44, borderRadius: '50%', background: '#1a1a1a', border: '1px solid #262626', color: '#fff', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>+</button>
        <input 
          type="text" 
        ...
        />
        <button style={{ width: 44, height: 44, borderRadius: '50%', background: '#7c3aed', border: 'none', color: '#fff', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>➤</button>
      </div>
