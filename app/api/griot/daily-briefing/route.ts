...
function buildDailyBriefing() {
  ...
  const briefings = [
    ...
    { message: 'The village has spoken in your honour at the Soro-Soke circle.', chips: [{ label: 'Join Circle', action: 'soro-soke' }, { label: 'Show Gratitude', action: 'react' }] },
  ]
  const picked = briefings[day]
  return { message: picked.message, chips: picked.chips, proverb: 'Umuntu ngumuntu ngabantu.', proverbLang: 'Zulu' }
}
